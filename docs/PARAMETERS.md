...
创建订单 `data.risk` 由 SDK 自动使用。仅 `enabled === true` 的厂商会采集；失败不阻断支付。

| 块            | 上送字段                        | 可覆盖配置（有值覆盖 SDK 默认）          |
| ------------- | ------------------------------- | ---------------------------------------- |
| `fingerprint` | `risk.fingerprint.visitorId`    | `apiKey`、`scriptUrlPattern`、`endpoint` |
...
`fingerprint` / `forter` / `checkout` 可只传 `{ enabled: true }` 使用内置默认。
